...
from observability.payment_intent import BRIEFING_TAG, BRIEFING_USD, resolve_payment_intent
...
def _find_unlocked_products(treasury: str, cycle_id: int) -> Set[str]:
    """Scan treasury for briefing unlocks (tag 2, plain memo, or JSON product_id)."""
    ...
    for entry in query_recent_transactions(treasury, limit=INGEST_LIMIT):
        ...
        if intent and intent.product_id:
            unlocked.add(intent.product_id)
    return unlocked
...
class PaidBriefing(RevenueEngine):
  ...
  <section id="unlock" style="border:2px dashed #888;padding:1rem;margin:1.5rem 0">
    <h2>Unlock Full Briefing — ${BRIEFING_USD:.2f}</h2>
    <p>Send testnet XRP to <code>{treasury}</code> with <strong>Destination Tag {BRIEFING_TAG}</strong>
       (or memo <code>briefing</code>). No JSON needed.</p>
...
"""
    ...
    def run(self, cycle_id: int) -> Dict[str, Any]:
        ...
        unlocked_products = _find_unlocked_products(treasury, cycle_id)
